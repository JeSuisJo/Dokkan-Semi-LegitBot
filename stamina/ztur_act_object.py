...
FOOD_CLICK_POSITION = (563, 658)
OK_AFTER_FOOD_1_ZONE = (496, 723, 580, 783)
OK_AFTER_FOOD_2_ZONE = (491, 628, 572, 687)
OK_AFTER_FOOD_3_ZONE = (355, 585, 443, 644)
DRAGON_STONE_CLICK_POSITION = (581, 431)
OK_AFTER_DRAGON_STONE_1_ZONE = (493, 627, 577, 688)
OK_AFTER_DRAGON_STONE_2_ZONE = (360, 587, 446, 645)

class NoStaminaException(Exception):
    pass
...
def wait_for_ok_and_click(adb_path, device, screenshot_file, ok_zone):
    while True:
        capture_screenshot(adb_path, device, screenshot_file)
        img = Image.open(screenshot_file)
        
        ok_pos = find_text_ok_in_zone(img, ok_zone)
        if ok_pos:
...
def use_items(adb_path, device):
    ...
    if config.get("use_meat_first", False):
        ...
        try:
            wait_for_ok_and_click(adb_path, device, screenshot_file, OK_AFTER_FOOD_1_ZONE)
            wait_for_ok_and_click(adb_path, device, screenshot_file, OK_AFTER_FOOD_2_ZONE)
            wait_for_ok_and_click(adb_path, device, screenshot_file, OK_AFTER_FOOD_3_ZONE)
        finally:
            if os.path.exists(screenshot_file):
                os.remove(screenshot_file)
    
    elif config.get("use_dragon_stones", False):
        ...
        try:
            wait_for_ok_and_click(adb_path, device, screenshot_file, OK_AFTER_DRAGON_STONE_1_ZONE)
            wait_for_ok_and_click(adb_path, device, screenshot_file, OK_AFTER_DRAGON_STONE_2_ZONE)
        finally:
            if os.path.exists(screenshot_file):
                os.remove(screenshot_file)
    
    else:
        print("No stamina recovery method available, stopping script")
        raise NoStaminaException("No stamina recovery method available")
