...
ZTUR_CLICK_POSITION = (401, 728)
ZTUR_CLICK_POSITION_ALT = (399, 749)
START_LEVEL_COLOR_POINT = (670, 1032)
START_LEVEL_COLOR = (255, 0, 0)
OK_TOO_MANY_FRIENDS_ZONE = (356, 656, 445, 717)
BACK_CLICK_POSITION = (401, 15)
OK_FRIEND_REQUEST_ZONE = (488, 655, 584, 715)
CANCEL_FRIEND_REQUEST_CLICK = (261, 686)
OK_LEVEL_COMPLETE_ZONE = (362, 988, 441, 1046)
...
def get_target_level():
    while True:
        try:
            level = input("At which level do you want to finish? (Enter a number): ").strip()
            level = int(level)
            if level > 0:
                return level
            else:
...
def get_enemy_level():
    while True:
        try:
            level = input("What is the enemy level? (Enter a number): ").strip()
            level = int(level)
            if level > 0:
                return level
            else:
...
def run_ztur_finish(num_runs):
    ...
    num_repetitions = (target_level + 1) - enemy_level
    
    print(f"\nZTUR Finish mode - until level {target_level}")
            ...
            while True:
                ...
                if ztur_pos:
                    ...
                    while True:
                        ...
                        if check_color_at_point(img, START_LEVEL_COLOR_POINT, START_LEVEL_COLOR):
                            print(f"Start level")
                            tap_on_device(ADB_PATH, device, START_LEVEL_COLOR_POINT[0], START_LEVEL_COLOR_POINT[1])
                            time.sleep(0.4)
                        ...
                        if ztur_pos_alt:
                            tap_on_device(ADB_PATH, device, ZTUR_CLICK_POSITION_ALT[0], ZTUR_CLICK_POSITION_ALT[1])
                            time.sleep(0.4)
                            continue
